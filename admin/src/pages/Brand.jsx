...
const Brand = () => {
  ...
  const { data, loading, error } = useAsync(() =>
    BrandServices.getAllBrand({ email: adminInfo.email })
  );
  ...
  const syncBrand = async () => {
    setSyncLoading(true);
    try {
      const res = await BrandServices.syncBrand();
      notifySuccess(res.message || "Brands synced successfully");
      setIsUpdate(true);
    } catch (err) {
      console.error(err);
      const errorMessage =
        err.response?.data?.message || err.message || "Failed to sync brands";
        ...
        <Card className="min-w-0 shadow-xs overflow-hidden bg-white dark:bg-gray-800 mb-5">
          <CardBody>
            ...
            >
              ...
              <div className="mt-2 md:mt-0 flex brands-center xl:gap-x-4 gap-x-1 flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                <Button
                ...
                <div className="w-full">
                  <Button
                    ...
                    type="button"
                    className="px-4 md:py-1 py-3 text-sm border border-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900 dark:border-blue-400 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
      ...
      {loading ? (
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          <Table>
            ...
            <BrandTable dataTable={dataTable} lang={lang} />
          </Table>
          <TableFooter>
            <Pagination
