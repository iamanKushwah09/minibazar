...
import VendorGroupDrawer from "@/components/drawer/VendorGroupDrawer";
import TableLoading from "@/components/preloader/TableLoading";
import VendorGroupTable from "@/components/vendorgroup/VendorGroupTable";
...
const VendorGroup = () => {
  ...
  const { data, loading, error } = useAsync(() =>
    VendorGroupServices.getAllVendorGroup({ email: adminInfo.email })
  );
  const [syncLoading, setSyncLoading] = useState(false);

  // refreshVendorGroup

  const refreshVendorGroup = async () => {
    try {
      setSyncLoading(true);
      const res = await VendorGroupServices.refreshVendorGroup();
  ...
  return (
    <>
      <PageTitle>{t("Vendor Groups")} </PageTitle>
      <MainDrawer>
        <VendorGroupDrawer />
      </MainDrawer>
      <AnimatedContent>
            ...
            >
              <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
                <Input
                  ...
                  type="search"
                  name="search"
                  placeholder={t("Search Vendor Group")}
                />
              </div>
              {/* <div className="flex-grow-0 md:flex-grow lg:flex-grow xl:flex-grow">
      ...
      {loading ? (
        ...
        <TableContainer className="mb-8 rounded-b-lg">
          <Table>
            ...
            <VendorGroupTable dataTable={dataTable} lang={lang} />
          </Table>
          <TableFooter>
            <Pagination
