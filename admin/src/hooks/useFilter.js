...
const useFilter = (data, attributeData = []) => {
  ...
  const [attributeValueData, setAttributeValueData] = useState([]); //tableTable for showing on table according to filtering
  ...
  const serviceData = useMemo(() => {
    ...
    if (categoryType) {
      services = services.filter((search) => {
        ...
        const nameMatches = typeof name === "object"
          ? (name[lang] || name?.en || "").toLowerCase().includes(searchTerm)
    ...
    if (searchUser) {
      // Special handling for catalog search
      if (location.pathname === "/catalogs") {
        const searchTerm = searchUser.toLowerCase();
        services = services.filter((item) => {
          try {

            // Search in catalog_name
            const catalogName = (item.catalog_name || "").toLowerCase();

            // Search in category names - handle both populated and non-populated
            let categoryNames = "";
            if (Array.isArray(item.category_id)) {
              categoryNames = item.category_id.map(cat => {
                if (typeof cat === 'object' && cat !== null) {
                  // Handle multilingual names
                  if (typeof cat.name === 'object') {
                    return Object.values(cat.name).join(" ");
                  }
                  return cat.name || "";
                } else {
                  return String(cat || "");
            ...
            } else if (item.category_id && typeof item.category_id === 'object') {
              // Single populated category
              if (typeof item.category_id.name === 'object') {
                categoryNames = Object.values(item.category_id.name).join(" ").toLowerCase();
              } else {
                categoryNames = (item.category_id.name || "").toLowerCase();
              }
            }

            // Search in item group names - handle both populated and non-populated
            let itemGroupNames = "";
            if (Array.isArray(item.item_group_id)) {
              itemGroupNames = item.item_group_id.map(group => {
                if (typeof group === 'object' && group !== null) {
                  // Handle multilingual names
                  if (typeof group.name === 'object') {
                    return Object.values(group.name).join(" ");
                  }
                  return group.name || "";
                } else {
                  return String(group || "");
            ...
            } else if (item.item_group_id && typeof item.item_group_id === 'object') {
              // Single populated item group
              if (typeof item.item_group_id.name === 'object') {
                itemGroupNames = Object.values(item.item_group_id.name).join(" ").toLowerCase();
              } else {
                itemGroupNames = (item.item_group_id.name || "").toLowerCase();
              }
            }
            ...
            const matches = catalogName.includes(searchTerm) ||
              categoryNames.includes(searchTerm) ||
              itemGroupNames.includes(searchTerm) ||
        ...
        // Original user search logic for other pages
        services = services.filter(
          (search) =>
            (search?.name?.[lang] || search?.name || "").toLowerCase()
    ...
    // Sale order filtering
    if (searchSaleOrder) {
      services = services.filter((search) =>
        (search?.voucherNo?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase()) ||
        (search?.customer?.name?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase()) ||
        (search?.customer?.email?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase()) ||
        (search?.customer?.mobile?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase()) ||
        (search?.customer?.gst_no?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase()) ||
        (search?.salesman?.name?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase()) ||
        (search?.saleType?.name?.toLowerCase() || "").includes(searchSaleOrder.toLowerCase())
      );
    }
    if (time) {
  ...
  const serviceData1 = useMemo(() => {
    const date = new Date();
    date.setDate(date.getDate() - time);
    let services = attributeData?.map((el) => {
      const newDate = new Date(el?.updatedAt).toLocaleString("en-US", {
        timeZone: globalSetting?.default_time_zone,
      });
      const newObj = {
  ...
  }, [serviceData1, serviceData, currentPage, resultsPerPage]);
  ...
  const handleSubmitSaleOrder = (e) => {
    e.preventDefault();
    setSearchSaleOrder(saleOrderRef.current.value);
  };
