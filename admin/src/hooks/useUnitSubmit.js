...
const useUnitSubmit = (id) => {
  ...
  const onSubmit = async (data) => {
    ...
    //   const permissions = [checkedState]?.map(item => Object.values(item))
      ...
      if (id) {
        const res = await UnitServices.updateUnit(id, roleData);
        setIsUpdate(true);
        setIsSubmitting(false);
        notifySuccess(res.message);
        closeDrawer();
      } else {
        const res = await UnitServices.addUnit(roleData);
        setIsUpdate(true);
        setIsSubmitting(false);
        notifySuccess(res.message);
        closeDrawer();
  ...
  const getRoleData = async () => {
    try {
      const res = await UnitServices.getUnitById(id, {
