...
const useBrandSubmit = (id) => {
    ...
    const onSubmit = async (data) => {
        try {
            let img = newImage.base64File ? newImage : data?.image;
            // Check if image is required and missing
            // if (!id && !newImage.base64File && !imageUrl) return notifyError("Image must be selected");
            ...
            if (id) {
                const res = await BrandServices.updateBrand(id, roleData);
                setIsUpdate(true);
                setIsSubmitting(false);
                notifySuccess(res.message);
                closeDrawer();
            } else {
                const res = await BrandServices.addBrand(roleData);
                setIsUpdate(true);
                setIsSubmitting(false);
                notifySuccess(res.message);
                closeDrawer();
